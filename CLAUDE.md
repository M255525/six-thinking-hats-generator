...
- **手動勾選「強制重新生成」指定帽子**（`#forceRegenRow`，僅六頂依序模式顯示，`updateForceRegenVisibility()` 依 `state.mode` 切換）：上面的「自動續跑」只處理「未完成/失敗/過期」，使用者若想針對某頂已成功的結果重新生成（例如覺得某頂措辭不理想想再試一次），可以勾選對應帽子的 checkbox（`renderForceRegenRow()` 動態生成，樣式是 `.force-chip` 圓角勾選膠囊，勾選時背景填該帽子的顏色）。`getForceRegenSet()` 在點下生成按鈕當下讀取目前勾選狀態，被勾選的帽子在 `results.map()` 那段一律強制回傳 `null`（優先權高於「沿用先前成功結果」的判斷）；跑完那一輪後 `clearForceRegen(forceSet)` 只清掉這次實際處理過的勾選（不是清空全部），沒被勾的維持原狀。checkbox 狀態不存 `localStorage`——是「下一次生成」的一次性意圖，不是需要跨頁面重新整理保留的設定。已用 Playwright + mock `fetch` 驗證：勾選黑帽＋綠帽後只送出 2 次 API 呼叫，其餘 4 頂文字完全不變，生成完成後 6 個勾選框與視覺 `.checked` 樣式皆正確自動清空。
...
- **列印範圍**：只印「③ 送給 AI 生成分析」的結果——`#topicPanel`／`#promptPanel`／`#savedPanel`／跑馬燈／頂欄／按鈕群／連 ai-panel 自己的 `.panel-head`（③ 標題）與 `.panel-hint`（說明文字）都在 `@media print` 隱藏；改用 `#printHeader`（列印時才 `display:block`）取代，用 `innerHTML` 組出「品牌小字（`.ph-brand`）→ 問題主題大標（`.ph-topic`）→ 模式＋產出時間小字（`.ph-meta`）」三層，配一條藍色 `border-bottom` 當報告封面感的分隔線，而不是單純一行純文字。六頂依序模式的 `.hat-card` 是 `<details>`，收合狀態列印會是空的，`printAiBtn` 點擊時會先把全部 `.hat-card` 的 `open` 設成 `true` 再呼叫 `window.print()`。
- **精簡美觀的版面調整**（2026-08-31 使用者要求「格式要精簡美觀」後改版）：`.panel`（含 `.ai-panel`）在列印時拿掉卡片外框／陰影／內距，不再是「網頁截圖」感，改成像文件一樣的平鋪版面；`@page{margin:16mm 14mm}` 控制頁面留白；內文字體從等寬 `--font-mono` 改回 `--font-sans`（等寬字體讀起來像程式碼，不適合報告內文）；`.hat-table` 改成「橫線報告表」風格——只留表頭下方一條粗底線與每列一條淺灰分隔線，拿掉所有直線與表頭底色，最後一列不留分隔線；`.hat-card` 的彩色左邊框（`--hc-fill`，各帽子代表色）保留，是唯一在報告裡出現的顏色元素，其餘一律黑白灰階以確保黑白印表機也清楚。已用 CSSOM（`document.styleSheets` 找出 `@media print` 規則逐條核對 selector）驗證 23 條規則全數正確解析，未被瀏覽器判定語法錯誤而整段忽略；`window.print` 綁定的實際觸發與 `#printHeader` 產生的 HTML 結構已用 mock 驗證正確。
